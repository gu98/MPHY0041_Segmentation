...
for step in range(total_iter):
    ...
    if (step % int(n/size_minibatch)) == 0:
        if step != 0:
            duration = np.sum(times[step-int(n/size_minibatch):step])
            loss_epoch = np.mean(losses[step-int(n/size_minibatch):step])
            losses_epochs.append(loss_epoch)
            print('','','')
            print('','','')
            print('','','', 'Epoch Duration=', duration, '[s]')
            print('','','', 'Epoch Loss Avg=', loss_epoch)
            print('','','')
            print('','','')

        tf.print('------------------------------------------------------------')
        tf.print('EPOCH', int(step/(n/size_minibatch))+1)

    if (step % 1) == 0:
        tf.print('>','Step', step+1, ': Loss=', loss_train)
...
#epochs = np.linspace(1, int(total_iter/(n/size_minibatch)), num=int(total_iter/(n/size_minibatch)))
epochs = np.linspace(1, int(total_iter/(n/size_minibatch))-1, num=int(total_iter/(n/size_minibatch))-1)
...
np.savetxt('losses_epochs.csv', losses_epochs, delimiter=',')
